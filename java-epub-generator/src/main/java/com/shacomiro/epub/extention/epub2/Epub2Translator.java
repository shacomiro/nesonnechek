...
public class Epub2Translator {
	...
	public EpubFileInfo createEpub2(String uuid, String fileName, List<String> lines) {
		...
		String bookUuid;
		if (book.getMetadata().isAutoGeneratedId()) {
			bookUuid = book.getMetadata().getIdentifiers().get(0).getValue();
		} else {
			bookUuid = UUID.randomUUID().toString();
		}

		return new EpubFileInfo(bookUuid, bookFileName, bookFilePath);
	}
