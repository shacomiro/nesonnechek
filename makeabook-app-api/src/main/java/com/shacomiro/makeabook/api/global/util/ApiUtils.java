...
public class ApiUtils {
	...
	public static <T> ResponseEntity<?> success(T response,
			RepresentationModelAssemblerSupport<T, ?> representationModelAssembler, HttpStatus status) {
		return new ResponseEntity<>(representationModelAssembler.toModel(response).add(docsLink()), status);
	}

	public static <T> ResponseEntity<CollectionModel<?>> success(List<T> response,
			RepresentationModelAssemblerSupport<T, ?> representationModelAssembler, HttpStatus status) {
